...
(function(){
  ...
  var parse = function(terms){
    return terms.split(" ").map(function(s){
      return s.trim().length < 2 ? undefined : s.trim();
    });
  };//function
  ...
  _search = function(terms){
    ...
    currentThread = Thread.run(function*(){
      ...
      if (!dimensions) {
        ...
        var filter = {
          "and": ["build.type", "build.platform", "build.branch", "build.test", "build.suite"].map(function(k){
            return {
              "or": words.map(function(w){
